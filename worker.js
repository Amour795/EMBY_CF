/**
 * =================================================================================
 * Cloudflare Worker Emby 终极数据大屏版
 * 功能：PWA 原生体验 + 高精度实时测速 + 精准客户端解析 + 数据库容错防白屏
 * =================================================================================
 */

// 🔐 安全配置
const PANEL_PASSWORD = 'emby'; // 控制台访问密码
const ALLOWED_COUNTRIES = []; // 地区限制：为空数组 [] 则放行全球；填入 ['CN'] 则仅限大陆

// 核心参数
const SPEEDTEST_CHUNK = new Uint8Array(1024 * 1024); // 测速块大小
...
<body>
  ...
  <main class="page" id="main-content" style="display: none;">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 0.8rem;">
      ...
      <div class="panel">
        <div style="font-weight:700; font-size:0.9rem">📱 客户端分布</div>
    ...
    <div class="panel">
      ...
      <table>
        <thead><tr><th>IP地址</th><th>使用的客户端</th><th>时长</th></tr></thead>
  ...
  <div id="speedtest-modal" class="modal" hidden>
    ...
    <div class="modal-content">
      ...
      <div id="st-results">
        ...
        <div class="st-item"><span>CF 节点:</span><span id="st-colo">--</span></div>
  ...
  <script>
    ...
    async function loadData() {
      try {
        ...
        if (!payload.ok) {
          ...
          document.getElementById('db-warning').innerText = '⚠️ 数据库连接异常或缺少表结构: ' + (payload.error || '未知错误');
        } else {
        ...
        if(d.userStats.length === 0) document.getElementById('user-stats-body').innerHTML = '<tr><td colspan="3" style="text-align:center;color:var(--text-muted)">暂无数据记录</td></tr>';
    ...
    document.getElementById('st-start-btn').onclick = async () => {
      ...
      try {
        ...
        // 第四阶段：下行流媒体测速 (ReadableStream 实现实时刷新)
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/manifest.json') {
      return new Response(JSON.stringify({
        name: "Emby 大屏", short_name: "EmbyDash", start_url: "/", display: "standalone",
    ...
    if (ALLOWED_COUNTRIES.length > 0 && request.cf?.country && !ALLOWED_COUNTRIES.includes(request.cf.country)) {
      return new Response('Blocked by GeoIP Rules', { status: 403 });
    }
    ...
    if (url.pathname === '/trace') return new Response(JSON.stringify({ ip: request.headers.get('cf-connecting-ip'), colo: request.cf?.colo || 'N/A' }), { headers: { 'Cache-Control': 'no-store' } });
    ...
    } catch { return new Response('Invalid Request URL Format', { status: 400 }); }
    ...
    if (upstream.pathname.includes('/Playing/Progress')) {
      ctx.waitUntil(recordUserAudit(env, request.headers.get('cf-connecting-ip') || '未知IP', clientName, 10));
    }
...
async function recordBasicStats(env, type, client) {
  ...
  } catch (e) { console.error('Basic Stats DB Error:', e); }
}
...
async function handleStatsRequest(env) {
  if (!env.DB) return new Response(JSON.stringify({ok: false, enabled: false, error: "Cloudflare D1 Database binding missing"}));
  try {
